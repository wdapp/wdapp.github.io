...
class LiveAdaptive {
  ...
  addAPIFunction() {
    ...
    this.liveInterface.on(this.liveInterface.ONANSWER, (result) => {
      LiveInfo.answerInfo = LiveInfo.parseAnswerInfo(result, LiveInfo.getLoginInfoData('viewer', 'id'))
      hdScience.dispatch(hdScience.OnAnswer)
    })
    //问答接收提问
    this.liveInterface.on(this.liveInterface.ONQUESTION, (result) => {
      LiveInfo.questionInfo = LiveInfo.parseQuestionInfo(result, LiveInfo.getLoginInfoData('viewer', 'id'))
